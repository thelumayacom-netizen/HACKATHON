...
export interface AuthUser extends User {
  // Remove profile interface since we'll use user_metadata
}

export const authService = {
  // Keep your existing signUp - it already stores full_name in user_metadata
  async signUp(email: string, password: string, fullName?: string) {
    try {
      const { data, error } = await supabase.auth.signUp({
        email,
  ...
  // Keep your existing getCurrentUser
  async getCurrentUser() {
    try {
      const { data: { user }, error } = await supabase.auth.getUser();
  ...
  // Keep your existing getCurrentSession
  async getCurrentSession() {
    try {
      const { data: { session }, error } = await supabase.auth.getSession();
  ...
  // Remove getUserProfile and updateProfile functions since we won't use profiles table

  // Keep your existing onAuthStateChange
  onAuthStateChange(callback: (event: string, session: any) => void) {
    return supabase.auth.onAuthStateChange(callback);
  },

  // Keep your existing refreshSession and clearSession
  async refreshSession() {
    try {
      const { data, error } = await supabase.auth.refreshSession();
